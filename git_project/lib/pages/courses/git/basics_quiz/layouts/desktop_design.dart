              ...
              onPressed:
                  () => verifyAnswer(
                    ...
                    correctedAnswer: Components.optionsCard(
                      title: 'Explanation:',
                      caption:
                          '\ngit add script.py stages only the modified file.\ngit commit -a (option 3) stages tracked files but ignores data.csv (new).\ngit add . (option 4) stages everything, including data.csv.',
        ...
        child: Row(
          ...
          children: <Widget>[
            ...
            ElevatedButton(
              onPressed: () {
                verifyAnswer(
                  ...
                  correctedAnswer: Components.optionsCard(
                    title: 'Explanation:',
                    caption:
                        '\ngit rebase main (option 3) replays feature/login commits on top of main, creating a linear history.\nmerge --no-ff (option 1) forces a merge commit.\nmerge --squash (option 4) combines all commits into one.',
                    icon: Icon(Icons.check_box, color: Colors.green),
                    captionMaxLines: 6,
                  ),
                );
              ...
              child: AutoSizeText(
                GitCoursePage.lessons.keys.elementAt(2),
                minFontSize: 5,
                maxLines: 1,
                style: TextStyle(fontSize: 20, color: Colors.black),
              ),
              ...
              onPressed:
                  () => verifyAnswer(
                    ...
                    correctedAnswer: Components.optionsCard(
                      title: 'Explanation:',
                      caption:
                          '\ngit restore --staged unstages the file.\ngit reset --hard (option 2) deletes all uncommitted changes.\ngit checkout HEAD~1 (option 3) reverts to the previous commit’s version.',
                      icon: Icon(Icons.check_box, color: Colors.green),
                      captionMaxLines: 6,
                    ),
                  ),
  ...
  Widget stepperContent(int index) {
    if (index == 0) {
      return checkbox4OptionTest(
        question:
            'You have two files: "script.py" (modified) and "data.csv" (new). You want to commit only "script.py".\nWhich sequence is correct?',
        option1: codingCard(
        ...
        option3: codingCard('> git commit -a -m "Update script"'),
        option4: codingCard('> git add .\n> git commit --amend'),
      );
    }
    if (index == 1) {
      return checkbox4OptionTest(
        question:
            'You staged index.html but realized you shouldn’t commit it. How to unstage it without losing changes?',
        option1: codingCard('> git restore --staged index.html'),
        option2: codingCard('> git reset --hard HEAD'),
        option3: codingCard('> git checkout HEAD~1 index.html'),
        option4: codingCard('> git commit -m "Oops" && git revert HEAD'),
      );
    }
    if (index == 2) {
      return checkbox4OptionTest(
        question:
            'You created a "feature/login" branch, made commits, and want to merge it into "main" without a merge commit.\nWhich command achieves this?',
        option1: codingCard(
          '> git switch main\n> git merge --no-ff feature/login',
        ),
        option2: codingCard('> git switch main\n> git rebase feature/login'),
        option3: codingCard('> git switch feature/login\n> git rebase main'),
        option4: codingCard(
